...
export default function NotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      // /notifications is the mobile/user feed. The admin history must use the
      // establishment-scoped admin endpoint to persist/display sent notifications.
      const { data } = await apiClient.get<{ success: boolean; data: NotificationItem[]; pagination: { page: number; totalPages: number; total: number } }>('/notifications/admin', { params });
      ...
      <Modal open={sendModalOpen} onClose={() => setSendModalOpen(false)} title="Envoyer une notification" size="lg">
        <div className="space-y-4">
          ...
          <div><div className="mb-2 flex items-center justify-between"><label className="text-sm font-medium text-slate-700">Destinataires</label>{sendTargetType === 'users' && selectedUserIds.length > 0 && <span className="text-xs font-medium text-primary">{selectedUserIds.length} sélectionné(s)</span>}{sendTargetType === 'groups' && selectedGroupIds.length > 0 && <span className="text-xs font-medium text-primary">{selectedGroupIds.length} sélectionné(s)</span>}</div>
            ...
            {sendTargetType === 'users' ? <div className="space-y-2"><SearchBar onSearch={setUserSearch} placeholder="Rechercher un utilisateur..." /><div className="max-h-48 overflow-y-auto rounded-md border border-line">{filteredUsers.map((u) => <label key={u.id} className="flex cursor-pointer items-center gap-3 border-b border-line px-3 py-2 last:border-0 hover:bg-slate-50"><input type="checkbox" checked={selectedUserIds.includes(u.id)} onChange={() => toggleUser(u.id)} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-slate-800">{u.name}</span><span className="block truncate text-xs text-muted">{u.email}</span></span></label>)}{!filteredUsers.length && <p className="p-3 text-sm text-muted">Aucun utilisateur trouvé.</p>}</div></div> : <div className="max-h-48 overflow-y-auto rounded-md border border-line">{groups.map((g) => <label key={g.id} className="flex cursor-pointer items-center gap-3 border-b border-line px-3 py-2 last:border-0 hover:bg-slate-50"><input type="checkbox" checked={selectedGroupIds.includes(g.id)} onChange={() => toggleGroup(g.id)} /><span className="text-sm font-medium text-slate-800">{g.name}</span></label>)}{!groups.length && <p className="p-3 text-sm text-muted">Aucun groupe trouvé.</p>}</div>}
          </div>
          <Input label="Titre" value={sendTitle} onChange={(e) => setSendTitle(e.target.value)} error={sendErrors.title} placeholder="Titre de la notification" />
          <div><label className="mb-1.5 block text-sm font-medium text-slate-700">Contenu</label><textarea value={sendBody} onChange={(e) => setSendBody(e.target.value)} rows={4} className={`w-full rounded-md border px-3 py-2 text-sm outline-none transition focus:ring-2 focus:ring-primary/20 ${sendErrors.body ? 'border-red-300' : 'border-line'}`} placeholder="Contenu de la notification" />{sendErrors.body && <p className="mt-1 text-sm text-red-600">{sendErrors.body}</p>}</div>
          <div className="flex justify-end gap-2 border-t border-line pt-4"><Button variant="secondary" onClick={() => setSendModalOpen(false)}>Annuler</Button><Button onClick={handleSend} loading={sending}><Send className="h-4 w-4" /> Envoyer</Button></div>
        </div>
      </Modal>
    </div>
  );
