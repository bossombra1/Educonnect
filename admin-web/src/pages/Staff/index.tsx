...
const emptyForm = { firstName: '', lastName: '', email: '', phone: '', role: 'STAFF' as UserRole, fonction: '', departement: '', password: '' };
...
export default function StaffPage() {
  ...
  const fetchStaff = useCallback(async () => {
    setLoading(true);
    try {
      const params: Record<string, string | number> = { page: pagination.page, limit: 20, role: 'STAFF,TEACHER' };
  ...
  const openEdit = (u: User) => {
    setEditingUser(u);
    setForm({ firstName: u.firstName, lastName: u.lastName, email: u.email, phone: u.phone || '', role: u.role, fonction: (u as any).fonction || '', departement: (u as any).departement || '', password: '' });
    setFormErrors({});
    setModalOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try { await userService.deleteUser(deleteTarget.id); toast.success('Supprimé'); setDeleteTarget(null); fetchStaff(); }
    catch {}
  ...
  const columns: Column<User>[] = [
    ...
    { key: 'fonction', header: 'Fonction', render: (u) => <span className="text-sm text-gray-600">{(u as any).fonction || '—'}</span> },
    { key: 'departement', header: 'Département', render: (u) => <span className="text-sm text-gray-600">{(u as any).departement || '—'}</span> },
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
          <div className="w-full sm:max-w-xs"><SearchBar onSearch={setSearch} placeholder="Rechercher du personnel..." /></div>
        ...
        <Button onClick={openCreate}><Plus className="h-4 w-4" /> Ajouter du personnel</Button>
      </div>

      <Card className="!p-0 overflow-hidden">
        <Table columns={columns} data={staff as any} loading={loading} keyExtractor={(u) => u.id} emptyMessage="Aucun personnel trouvé" />
      </Card>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingUser ? 'Modifier le personnel' : 'Ajouter du personnel'} size="lg">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Input label="Fonction" value={form.fonction} onChange={(e) => setForm({ ...form, fonction: e.target.value })} placeholder="Ex: Professeur de maths" />
          <Input label="Département" value={form.departement} onChange={(e) => setForm({ ...form, departement: e.target.value })} placeholder="Ex: Sciences" />
      ...
      <ConfirmDialog open={!!deleteTarget} title="Supprimer" message={`Voulez-vous vraiment supprimer ${deleteTarget?.firstName} ${deleteTarget?.lastName} ?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} variant="danger" loading={deleting} />
    </div>
  );
}