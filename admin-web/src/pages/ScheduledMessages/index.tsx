...
const statusBadge: Record<string, 'warning' | 'info' | 'success' | 'danger'> = { pending: 'warning', processing: 'info', sent: 'success', failed: 'danger', cancelled: 'info' };
...
export default function ScheduledMessagesPage() {
  ...
  const columns: Column<ScheduledMessage>[] = [
    ...
    { key: 'status', header: 'Statut', render: (sm) => <Badge variant={statusBadge[sm.status] || 'info'}>{statusLabel[sm.status] || sm.status}</Badge> },
  ...
  return <div className="space-y-4"><div className="flex flex-col gap-2 border-b border-line pb-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-lg font-semibold tracking-tight text-slate-900">Messages programmés</h1><p className="mt-0.5 text-xs text-muted">{messages.length} message(s) programmé(s)</p></div><Button variant="ghost" onClick={() => void fetchScheduled()}><RefreshCw className="h-4 w-4" /> Actualiser</Button></div>{loadError && <div role="alert" className="flex items-center justify-between gap-3 rounded-md border border-red-200 bg-red-50 px-3 py-2.5 text-xs text-red-700"><span>{loadError}</span><Button variant="secondary" onClick={() => void fetchScheduled()}>Réessayer</Button></div>}<Card className="!p-0 overflow-hidden"><Table columns={columns} data={messages} loading={loading} keyExtractor={(sm) => sm.id} emptyMessage={loadError ? 'Impossible de charger les messages' : 'Aucun message programmé'} /></Card><Modal open={!!detailMsg} onClose={() => setDetailMsg(null)} title={detailMsg?.title || 'Détails du message'} size="lg">{detailLoading ? <div className="py-8 text-center text-xs text-slate-400">Chargement...</div> : detailError ? <div role="alert" className="space-y-3 py-6 text-center"><p className="text-xs text-red-600">{detailError}</p><Button variant="secondary" onClick={() => detailMsg && void showDetail({ id: detailMsg.id, message: detailMsg, scheduledAt: detailMsg.scheduledAt || detailMsg.createdAt, status: 'pending', retryCount: 0 })}>Réessayer</Button></div> : detailMsg ? <div className="space-y-4"><div><span className="text-xs font-medium text-muted">Contenu :</span><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-900">{detailMsg.content}</p></div><div className="grid grid-cols-1 gap-3 border-y border-line py-3 text-xs sm:grid-cols-2"><div><span className="text-muted">Type :</span> <span className="font-medium text-slate-900">{detailMsg.type}</span></div><div><span className="text-muted">Priorité :</span> <span className={getPriorityColor(detailMsg.priority)}>{getPriorityLabel(detailMsg.priority)}</span></div><div><span className="text-muted">Destinataires :</span> <span className="font-medium text-slate-900">{detailMsg.totalRecipients}</span></div><div><span className="text-muted">Taux de lecture :</span> <span className="font-medium text-slate-900">{detailMsg.totalRecipients > 0 ? ((detailMsg.readCount / detailMsg.totalRecipients) * 100).toFixed(1) : 0}%</span></div></div>{detailMsg.attachments.length > 0 && <div><span className="text-xs font-medium text-muted">Pièces jointes :</span><div className="mt-2 space-y-1">{detailMsg.attachments.map(a => <div key={a.id} className="rounded-md border border-line bg-slate-50 px-3 py-2 text-xs text-slate-700">📎 {a.filename}</div>)}</div></div>}</div> : null}</Modal><ConfirmDialog open={!!cancelTarget} title="Annuler le message" message="Voulez-vous vraiment annuler l'envoi programmé de ce message ?" onConfirm={handleCancel} onCancel={() => setCancelTarget(null)} loading={cancelling} /></div>;
}
