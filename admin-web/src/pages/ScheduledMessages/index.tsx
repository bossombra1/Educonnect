...
export default function ScheduledMessagesPage() {
  ...
  const fetchScheduled = useCallback(async () => {
    setLoading(true);
    try {
      const res = await messageService.getMessages({ status: 'pending' });
      setMessages((res.data || []).map((m: Message) => ({
        id: m.id,
        message: m,
        scheduledAt: m.scheduledAt || m.createdAt,
        status: m.status === 'pending' ? 'pending' : m.status === 'sent' ? 'sent' : m.status === 'failed' ? 'failed' : 'pending',
        retryCount: 0,
      })));
    } catch { setMessages([]); }
  ...
  const showDetail = async (sm: ScheduledMessage) => {
    setDetailLoading(true);
    setDetailMsg(sm.message);
    try {
      const msg = await messageService.getMessage(sm.id);
      setDetailMsg(msg);
    } catch {}
  ...
  const columns: Column<ScheduledMessage>[] = [
    ...
    { key: 'actions', header: 'Actions', className: 'text-right', render: (sm) => (
      <div className="flex items-center justify-end gap-1">
        <button onClick={() => showDetail(sm)} className="rounded-lg p-1.5 text-gray-400 hover:bg-blue-50 hover:text-primary"><Eye className="h-4 w-4" /></button>
        {sm.status === 'pending' && (
          <button onClick={() => setCancelTarget(sm)} className="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600"><XCircle className="h-4 w-4" /></button>
        )}
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{messages.length} message(s) programmé(s)</p>
        <Button variant="ghost" onClick={fetchScheduled}><RefreshCw className="h-4 w-4" /> Actualiser</Button>
      </div>

      <Card className="!p-0 overflow-hidden">
        <Table columns={columns} data={messages as any} loading={loading} keyExtractor={(sm) => sm.id} emptyMessage="Aucun message programmé" />
      </Card>
      ...
      <Modal open={!!detailMsg} onClose={() => setDetailMsg(null)} title={detailMsg?.title || 'Détails du message'} size="lg">
        {detailLoading ? (
          ...
          <div className="space-y-4">
            <div><span className="text-sm font-medium text-gray-500">Contenu :</span><p className="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{detailMsg.content}</p></div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-gray-500">Type :</span> <span className="font-medium">{detailMsg.type}</span></div>
              ...
              <div><span className="text-gray-500">Taux de lecture :</span> <span className="font-medium">{detailMsg.totalRecipients > 0 ? ((detailMsg.readCount / detailMsg.totalRecipients) * 100).toFixed(1) : 0}%</span></div>
            </div>
            {detailMsg.attachments.length > 0 && (
              <div><span className="text-sm font-medium text-gray-500">Pièces jointes :</span>
