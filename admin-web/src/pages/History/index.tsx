...
export default function HistoryPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {showFilters && (
        <Card>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div><label className="mb-1 block text-xs font-medium text-muted">Type</label><select value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(1); }} className="w-full rounded-md border border-line bg-white px-3 py-2 text-xs text-slate-900 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"><option value="">Tous</option><option value="text">Texte</option><option value="image">Image</option><option value="pdf">PDF</option><option value="link">Lien</option></select></div>
      ...
      <Card className="!p-0 overflow-hidden"><Table columns={columns} data={messages as any} loading={loading} keyExtractor={(m) => m.id} emptyMessage="Aucun message envoyé" /></Card>
      ...
      <Modal open={!!detailMsg} onClose={() => setDetailMsg(null)} title={detailMsg?.title || 'Détails du message'} size="lg">
        {detailLoading ? <div className="py-8 text-center text-xs text-slate-400">Chargement...</div> : detailError ? (
          <div role="alert" className="space-y-3 rounded-md border border-red-200 bg-red-50 p-4 text-xs text-red-700"><p>{detailError}</p><Button variant="secondary" size="sm" onClick={() => detailMsg && showDetail(detailMsg)}>Réessayer</Button></div>
        ) : detailMsg ? (
          <div className="space-y-4">
            <div><span className="text-xs text-muted">Contenu :</span><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-900">{detailMsg.content}</p></div>
            <div className="grid grid-cols-1 gap-3 border-y border-line py-3 text-xs sm:grid-cols-2">
              <div><span className="text-muted">Type :</span> <span className="font-medium text-slate-900">{typeLabel[detailMsg.type]}</span></div>
              <div><span className="text-muted">Priorité :</span> <span className={getPriorityColor(detailMsg.priority)}>{getPriorityLabel(detailMsg.priority)}</span></div>
              <div><span className="text-muted">Date :</span> <span className="font-medium text-slate-900">{formatDateTime(detailMsg.sentAt || detailMsg.createdAt)}</span></div>
              <div><span className="text-muted">Destinataires :</span> <span className="font-medium text-slate-900">{detailMsg.totalRecipients}</span></div>
              <div><span className="text-muted">Lus :</span> <span className="font-medium text-slate-900">{detailMsg.readCount} ({detailMsg.totalRecipients > 0 ? ((detailMsg.readCount / detailMsg.totalRecipients) * 100).toFixed(1) : 0}%)</span></div>
              <div><span className="text-muted">Livrés :</span> <span className="font-medium text-slate-900">{detailMsg.deliveryCount}</span></div>
            </div>
            {detailMsg.attachments.length > 0 && <div><span className="text-xs text-muted">Pièces jointes :</span><div className="mt-2 space-y-1">{detailMsg.attachments.map(a => <div key={a.id} className="rounded-md border border-line bg-slate-50 px-3 py-2 text-xs text-slate-700">📎 {a.filename}</div>)}</div></div>}
          </div>
        ) : null}
      </Modal>
    </div>
