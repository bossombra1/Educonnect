      ...
      <Modal open={!!detailId} onClose={closeDetail} title={detailMsg?.title || 'Détails du message'} size="lg">
        {detailLoading ? <div className="py-10 text-center text-xs text-slate-400">Chargement des détails et des destinataires...</div> : detailError ? (
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div><h3 className="mb-2 text-xs font-semibold text-slate-900">Pièces jointes</h3>{detailMsg.attachments.length ? <div className="space-y-1">{detailMsg.attachments.map((attachment) => <button type="button" key={attachment.id} onClick={() => void downloadProtectedFile(attachment.url, attachment.filename)} className="flex w-full items-center gap-2 rounded-md border border-line bg-slate-50 px-3 py-2 text-left text-xs text-primary hover:bg-slate-100"><Paperclip className="h-3.5 w-3.5" />{attachment.filename}</button>)}</div> : <p className="text-xs text-muted">Aucune pièce jointe</p>}</div>
