...
export default function StudentsPage() {
  ...
  useEffect(() => { classService.getClasses().then(setClasses).catch(() => {}); }, []);

  const openCreate = () => { setEditingUser(null); setForm(emptyForm); setFormErrors({}); setModalOpen(true); };
  const openEdit = (u: User) => {
    setEditingUser(u);
    setForm({ firstName: u.firstName, lastName: u.lastName, email: u.email, phone: u.phone || '', classId: '', matricule: '', password: '' });
    setFormErrors({});
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingUser) {
        ...
        await userService.createUser({ ...form, role: 'STUDENT' } as any);
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importService.importStudents(importFile);
      setImportResult(result);
      toast.success(`${result.success} élèves importés`);
      fetchStudents();
    } catch {}
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
          <div className="w-full sm:max-w-xs"><SearchBar onSearch={setSearch} placeholder="Rechercher un élève..." /></div>
      ...
      <Card className="!p-0 overflow-hidden">
        <Table columns={columns} data={students as any} loading={loading} keyExtractor={(u) => u.id} emptyMessage="Aucun élève trouvé" />
      </Card>
      ...
      {/* Import Modal */}
      <Modal open={importModalOpen} onClose={() => setImportModalOpen(false)} title="Importer des élèves" size="md">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Importez un fichier Excel (.xlsx) contenant la liste des élèves. Les colonnes attendues : Prénom, Nom, Email, Téléphone, Matricule.</p>
          <input type="file" accept=".xlsx,.xls" onChange={(e) => setImportFile(e.target.files?.[0] || null)} className="block w-full text-sm text-gray-500 file:mr-4 file:rounded-lg file:border-0 file:bg-primary/10 file:px-4 file:py-2 file:text-sm file:font-medium file:text-primary hover:file:bg-primary/20" />
          {importResult && (
            <div className="rounded-lg bg-gray-50 p-4">
              <p className="text-sm font-medium text-gray-700">Résultat : {importResult.success} réussis, {importResult.failed} échoués sur {importResult.total}</p>
              {importResult.errors.length > 0 && (
                <ul className="mt-2 max-h-32 overflow-y-auto text-xs text-red-600">
                  {importResult.errors.map((err, i) => <li key={i}>Ligne {err.row} : {err.message}</li>)}
                </ul>
              )}
            </div>
          )}
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setImportModalOpen(false)}>Fermer</Button>
            <Button onClick={handleImport} loading={importing} disabled={!importFile}>Importer</Button>
          </div>
        </div>
      </Modal>

      <ConfirmDialog open={!!deleteTarget} title="Supprimer l'élève" message={`Voulez-vous vraiment supprimer ${deleteTarget?.firstName} ${deleteTarget?.lastName} ?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} variant="danger" loading={deleting} />
    </div>
  );
}