...
export default function ClassesPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3,4,5,6].map(i => <div key={i} className="h-36 animate-pulse rounded-xl bg-gray-100" />)}</div> : !loadError && classes.length === 0 ? <Card><div className="flex flex-col items-center justify-center py-12 text-gray-400"><BookOpen className="mb-2 h-8 w-8" /><p className="text-sm">Aucune classe trouvée</p></div></Card> : !loadError && <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{classes.map(cls => <Card key={cls.id} className="relative"><div className="flex items-start justify-between"><div><h3 className="text-lg font-semibold text-gray-900">{cls.name}</h3><p className="text-sm text-gray-500">{cls.level} {cls.section ? `- ${cls.section}` : ''}</p></div><div className="flex gap-1"><button onClick={() => openEdit(cls)} className="rounded-lg p-1.5 text-gray-400 hover:bg-blue-50 hover:text-primary" aria-label={`Modifier ${cls.name}`}><Pencil className="h-4 w-4" /></button><button onClick={() => setDeleteTarget(cls)} className="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600" aria-label={`Supprimer ${cls.name}`}><Trash2 className="h-4 w-4" /></button></div></div><div className="mt-4 flex items-center gap-4 text-sm text-gray-500"><div className="flex items-center gap-1.5"><Users className="h-4 w-4" /> {cls.studentCount ?? 0} / {cls.capacity} élèves</div><span>Année {cls.schoolYear}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-gray-100"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.min(100, ((cls.studentCount ?? 0) / Math.max(1, cls.capacity)) * 100)}%` }} /></div></Card>)}</div>}
