...
import { Pencil, Trash2, Plus, Users, UsersRound } from 'lucide-react';
...
export default function GroupsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try { await groupService.deleteGroup(deleteTarget.id); toast.success('Groupe supprimé'); setDeleteTarget(null); fetchData(); }
    catch {}
  ...
  const columns: Column<Group>[] = [
    ...
    { key: 'memberCount', header: 'Membres', render: (g) => (
      <div className="flex items-center gap-1.5"><Users className="h-4 w-4 text-gray-400" /><span className="text-sm text-gray-700">{g.memberCount}</span></div>
    )},
    ...
    { key: 'actions', header: 'Actions', className: 'text-right', render: (g) => (
      <div className="flex items-center justify-end gap-1">
        <button onClick={() => openEdit(g)} className="rounded-lg p-1.5 text-gray-400 hover:bg-blue-50 hover:text-primary"><Pencil className="h-4 w-4" /></button>
        <button onClick={() => setDeleteTarget(g)} className="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="!p-0 overflow-hidden">
        <Table columns={columns} data={groups as any} loading={loading} keyExtractor={(g) => g.id} emptyMessage="Aucun groupe trouvé" />
      </Card>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Modifier le groupe' : 'Nouveau groupe'} size="lg">
        <div className="space-y-4">
          ...
          {form.type === 'class' && (
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700">Classes</label>
              <div className="max-h-40 overflow-y-auto rounded-lg border border-gray-200 p-2 space-y-1">
                {classes.map(c => (
                  <label key={c.id} className="flex items-center gap-2 rounded px-2 py-1.5 hover:bg-gray-50 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-gray-700">{c.name} ({c.studentCount ?? 0} élèves)</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          {form.type === 'level' && (
            <Select label="Niveau" options={[
              { value: 'CP1', label: 'CP1' }, { value: 'CP2', label: 'CP2' }, { value: 'CE1', label: 'CE1' },
              { value: 'CE2', label: 'CE2' }, { value: 'CM1', label: 'CM1' }, { value: 'CM2', label: 'CM2' },
              { value: '6ème', label: '6ème' }, { value: '5ème', label: '5ème' }, { value: '4ème', label: '4ème' },
              { value: '3ème', label: '3ème' }, { value: '2nde', label: '2nde' }, { value: '1ère', label: '1ère' },
              { value: 'Terminale', label: 'Terminale' },
            ]} value={form.level} onChange={(e) => setForm({ ...form, level: e.target.value })} placeholder="Sélectionner" />
          )}
          ...
          {form.type === 'role' && (
            ...
            ]} value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="Sélectionner" />
          )}

          {form.type === 'all_school' && (
            <div className="rounded-lg bg-blue-50 p-4 text-sm text-blue-700">
              <UsersRound className="mb-1 h-5 w-5" /> Ce groupe inclura tous les utilisateurs de l\'établissement.
            </div>
          )}

          <div className="rounded-lg bg-gray-50 p-3 text-sm text-gray-600">
            <strong>Aperçu :</strong> {getPreviewCount()}
          </div>
        </div>

        <div className="mt-6 flex justify-end gap-3">
          <Button variant="secondary" onClick={() => setModalOpen(false)}>Annuler</Button>
          <Button onClick={handleSave} loading={saving}>{editing ? 'Enregistrer' : 'Créer le groupe'}</Button>
        </div>
      </Modal>
