...
export default function ParentsPage() {
  const [parents, setParents] = useState<User[]>([]); const [pagination, setPagination] = useState({ page: 1, totalPages: 1, total: 0 }); const [loading, setLoading] = useState(true); const [search, setSearch] = useState(''); const [modalOpen, setModalOpen] = useState(false); const [childrenModal, setChildrenModal] = useState<User | null>(null); const [children, setChildren] = useState<User[]>([]); const [childrenLoading, setChildrenLoading] = useState(false); const [editingUser, setEditingUser] = useState<User | null>(null); const [form, setForm] = useState<FormData>({ ...emptyForm }); const [formErrors, setFormErrors] = useState<Partial<Record<keyof FormData, string>>>({}); const [deleteTarget, setDeleteTarget] = useState<User | null>(null); const [deleting, setDeleting] = useState(false); const [saving, setSaving] = useState(false);

  const fetchParents = useCallback(async () => { setLoading(true); try { const params: Record<string,string|number>={ page:pagination.page,limit:20,role:'PARENT' }; if(search)params.search=search; const res=await userService.getUsers(params as any); setParents(res.data); setPagination({page:res.pagination.page,totalPages:res.pagination.totalPages,total:res.pagination.total}); } catch(error){setParents([]);toast.error(error instanceof Error?error.message:'Impossible de charger les parents');} finally{setLoading(false);} },[pagination.page,search]);
  ...
  const showChildren=async(parent:User)=>{setChildrenModal(parent);setChildrenLoading(true);try{setChildren(await userService.getStudentsByParent(parent.id));}catch(error){setChildren([]);toast.error(error instanceof Error?error.message:'Impossible de charger les enfants');}finally{setChildrenLoading(false);}};
  ...
  const columns:Column<User>[]=[
    ...
    {key:'children',header:'Enfants',render:u=><button onClick={()=>showChildren(u)} className="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-2 py-1 text-xs font-medium text-primary hover:bg-blue-100"><Eye className="h-3 w-3"/>Voir</button>},
  ...
  return <div className="space-y-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="w-full sm:max-w-xs"><SearchBar onSearch={v=>{setSearch(v);setPagination(p=>({...p,page:1}));}} placeholder="Rechercher un parent..."/></div><Button onClick={openCreate}><Plus className="h-4 w-4"/>Ajouter un parent</Button></div>
    ...
    <Modal open={!!childrenModal} onClose={()=>setChildrenModal(null)} title={`Enfants de ${childrenModal?.firstName} ${childrenModal?.lastName}`}>{childrenLoading?<div className="py-8 text-center text-gray-400">Chargement...</div>:children.length===0?<div className="py-8 text-center text-gray-400">Aucun enfant trouvé</div>:<div className="space-y-2">{children.map(c=><div key={c.id} className="flex items-center gap-3 rounded-lg border border-gray-100 p-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-50 text-sm font-medium text-amber-700">{c.firstName[0]}{c.lastName[0]}</div><div><p className="text-sm font-medium text-gray-900">{c.firstName} {c.lastName}</p><p className="text-xs text-gray-400">{c.email||'—'}</p></div></div>)}</div>}</Modal>
