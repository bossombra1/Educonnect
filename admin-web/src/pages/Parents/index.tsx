...
export default function ParentsPage() {
  ...
  const columns:Column<User>=[
    ...
    {key:'children',header:'Enfants',render:u=><button aria-label={`Voir les enfants de ${u.firstName} ${u.lastName}`} onClick={()=>showChildren(u)} className="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-2 py-1 text-xs font-medium text-primary hover:bg-blue-100"><Eye className="h-3 w-3"/>Voir</button>},
  ...
  return <div className="space-y-4">
    ...
    <Modal open={!!childrenModal} onClose={()=>{setChildrenModal(null);setChildrenError(null);}} title={`Enfants de ${childrenModal?.firstName} ${childrenModal?.lastName}`}>{childrenLoading?<div className="py-8 text-center text-gray-400">Chargement...</div>:childrenError?<div role="alert" className="space-y-3 py-6 text-center"><p className="text-sm text-red-600">{childrenError}</p><Button variant="secondary" onClick={()=>childrenModal&&showChildren(childrenModal)}>Réessayer</Button></div>:children.length===0?<div className="py-8 text-center text-gray-400">Aucun enfant trouvé</div>:<div className="space-y-2">{children.map(c=><div key={c.id} className="flex items-center gap-3 rounded-lg border border-gray-100 p-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-50 text-sm font-medium text-amber-700">{c.firstName[0]}{c.lastName[0]}</div><div><p className="text-sm font-medium text-gray-900">{c.firstName} {c.lastName}</p><p className="text-xs text-gray-400">{c.email||'—'}</p></div></div>)}</div>}</Modal>
