...
function normalizeRecipient(raw: any): MessageRecipient {
  ...
  return {
    ...
    user,
    status: raw.status || raw.deliveryStatus || raw.delivery_status || 'pending',
    interactionStatus: raw.interactionStatus || raw.interaction_status,
    firstName: raw.firstName || raw.first_name || user?.firstName,
    lastName: raw.lastName || raw.last_name || user?.lastName,
    matricule: raw.matricule || user?.matricule,
    phone: raw.phone || user?.phone,
    role: raw.role || user?.role,
    schoolMatricule: raw.schoolMatricule || raw.school_matricule,
    studentStatus: raw.studentStatus || raw.student_status,
...
function normalizeMessage(raw: any): Message {
  const sender = raw.sender || (raw.sender_first_name || raw.sender_last_name || raw.senderFirstName || raw.senderLastName
    ? {
        ...
        role: raw.senderRole || raw.sender_role || 'STAFF',
        isActive: true,
        createdAt: '',
        updatedAt: '',
      } as User
  ...
  return {
    ...
    deliveryCount: Number(raw.deliveryCount ?? raw.delivery_count ?? raw.delivered ?? 0),
...
export const messageService = {
  ...
  async getMessageHistoryDetail(id: string): Promise<Message> {
    const { data } = await apiClient.get<ApiResponse<any>>(`/messages/history/${id}`);
    return normalizeMessage(data.data);
  },
