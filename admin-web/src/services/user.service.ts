...
function normalizeUser(raw: ApiUser): User {
  return {
    ...
    matricule: raw.matricule ?? raw.matricule_scolaire ?? undefined,
...
export const userService = {
  ...
  async createUser(userData: Partial<User> & { password: string }): Promise<User> {
    const { data } = await apiClient.post<ApiResponse<ApiUser>>('/users', toApiPayload(userData as Record<string, any>));
    return normalizeUser(data.data);
  },

  async updateUser(id: string, userData: Partial<User>): Promise<User> {
    const { data } = await apiClient.put<ApiResponse<ApiUser>>(`/users/${id}`, toApiPayload(userData as Record<string, any>));
    return normalizeUser(data.data);
  },
  ...
  async searchUsers(query: string): Promise<User[]> {
    const { data } = await apiClient.get<ApiResponse<ApiUser[]>>('/users/search', { params: { q: query } });
