...
export async function importStudentsFromExcel(fileBuffer: Buffer, establishmentId: number, importedBy: number): Promise<ImportResult> {
  ...
  if (rows.length === 0) throw new Error('Le fichier Excel ne contient aucune donnée après les en-têtes.');
  for (const col of REQUIRED_COLUMNS) if (!(col in rows[0])) throw new Error(`Colonne manquante: ${col}`);
  ...
  if (uniqueMatricules.length > 0) {
    ...
    for (let i = 0; i < rows.length; i++) {
      const matricule = (rows[i].Matricule || '').toString().trim();
      if (existingMatricules.has(matricule)) result.errors.push({ row: headerRow + i + 2, message: `Matricule déjà existant: ${matricule}` });
    }
    result.failCount += rows.filter(row => existingMatricules.has((row.Matricule || '').toString().trim())).length;
  }
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const [studentUser] = await conn.query<ResultSetHeader>('INSERT INTO users (establishment_id, role_id, matricule, first_name, last_name, phone, password_hash, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)', [establishmentId, studentRoleId, matricule, firstName, lastName, null, studentPassword]);
        ...
        result.successCount++;
      } catch (err) { result.errors.push({ row: excelRowNumber, message: `Erreur: ${(err as Error).message}` }); result.failCount++; }
    }
...
export async function previewImport(fileBuffer: Buffer): Promise<{ totalRows: number; columns: string[]; sampleRows: Record<string, any>[] }> {
  const workbook = XLSX.read(fileBuffer, { type: 'buffer' }); const sheetName = workbook.SheetNames[0]; const sheet = workbook.Sheets[sheetName!]; const headerRow = findHeaderRow(sheet); const rows = XLSX.utils.sheet_to_json<Record<string, any>>(sheet, { range: headerRow, defval: '' }); return { totalRows: rows.length, columns: rows.length > 0 ? Object.keys(rows[0]) : [], sampleRows: rows.slice(0, 5) };
}
