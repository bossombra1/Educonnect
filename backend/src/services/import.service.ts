...
export async function importStudentsFromExcel(fileBuffer: ArrayBuffer, establishmentId: number, importedBy: number): Promise<ImportResult> {
  ...
  for (const col of requiredColumns) if (!(col in rows[0])) throw new Error(`Colonne manquante: ${col}`);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const [studentUser] = await conn.query<ResultSetHeader>(
          ...
          [establishmentId, studentRoleId, matricule, firstName, lastName, telParent1 || null, studentPassword]
        );
        ...
        if ((telParent1 || telParent2) && parentRoleId) {
          ...
          const [parentUser] = await conn.query<ResultSetHeader>(
            'INSERT INTO users (establishment_id, role_id, matricule, first_name, last_name, phone, password_hash, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)',
            [establishmentId, parentRoleId, parentMatricule, 'Parent', lastName, telParent1 || null, parentPassword]
          );
          ...
          if (telParent2 && telParent2 !== telParent1) {
            ...
            const [parent2User] = await conn.query<ResultSetHeader>(
              'INSERT INTO users (establishment_id, role_id, matricule, first_name, last_name, phone, password_hash, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)',
              [establishmentId, parentRoleId, parent2Matricule, 'Parent', lastName, telParent2, parent2Password]
            );
...
export async function previewImport(fileBuffer: ArrayBuffer): Promise<{ totalRows: number; columns: string[]; sampleRows: Record<string, any>[] }> {
  ...
  return { totalRows: rows.length, columns: rows.length > 0 ? Object.keys(rows[0]) : [], sampleRows: rows.slice(0, 5) };
}
...
export async function getImportHistory(establishmentId: number, pagination: { page?: number; limit?: number }): Promise<{ data: any[]; total: number; page: number; limit: number; totalPages: number }> {
  ...
  const [imports] = await pool.query<RowDataPacket[]>(
    `SELECT i.*, u.email as imported_by_email
     FROM imports i
     LEFT JOIN users u ON u.id = i.imported_by AND u.establishment_id = i.establishment_id
     WHERE i.establishment_id = ? ORDER BY i.created_at DESC LIMIT ? OFFSET ?`,
