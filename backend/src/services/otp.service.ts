...
function phoneVariants(phone: string): string[] { const normalized = normalizePhone(phone); const local = normalized.startsWith('225') ? normalized.slice(3) : normalized; return Array.from(new Set([normalized, `+${normalized}`, `+225${local}`, local, `0${local.replace(/^0/, '')}`])); }
...
export async function requestOtp(identity: OtpIdentity): Promise<{ message: string; requiresChildMatricule?: boolean }> {
  ...
  await pool.query(`UPDATE users SET otp_code = ?, otp_expires_at = DATE_ADD(NOW(), INTERVAL ${OTP_TTL_MINUTES} MINUTE), otp_attempts = 0, otp_requested_at = NOW(), otp_verified = FALSE WHERE id = ?`, [otpHash, user.id]);
...
export async function verifyOtp(identity: OtpIdentity, code: string): Promise<{ token: string; user: any }> {
  ...
  if (!isValid) { await pool.query('UPDATE users SET otp_attempts = otp_attempts + 1 WHERE id = ?', [user.id]); throw new Error('Code OTP invalide.'); }
  await pool.query(`UPDATE users SET otp_code = NULL, otp_expires_at = NULL, otp_attempts = 0, otp_requested_at = NULL, otp_verified = TRUE, last_login_at = NOW() WHERE id = ?`, [user.id]);
  const payload: JwtPayload = { userId: user.id, role: user.role_name, establishmentId: user.establishment_id };
  const token = jwt.sign(payload, env.jwt.secret, { expiresIn: env.jwt.expiresIn as SignOptions['expiresIn'] });
  const userData: Record<string, any> = { id: user.id, matricule: user.matricule, first_name: user.first_name, last_name: user.last_name, phone: user.phone, role: user.role_name, establishment_id: user.establishment_id };
  if (user.role_name === 'PARENT') {
    const [children] = await pool.query<RowDataPacket[]>(`SELECT s.id AS student_id, u_s.first_name, u_s.last_name, s.matricule_scolaire, s.status AS student_status, c.name AS class_name, c.level, c.section, ps.priority, ps.is_emergency_contact FROM parents p JOIN parent_student ps ON ps.parent_id = p.id JOIN students s ON s.id = ps.student_id JOIN users u_s ON u_s.id = s.user_id LEFT JOIN classes c ON c.id = s.class_id WHERE p.user_id = ? ORDER BY u_s.last_name, u_s.first_name`, [user.id]);
    userData.children = children;
  }
  await pool.query('INSERT INTO audit_logs (user_id, action, entity_type, created_at) VALUES (?, ?, ?, NOW())', [user.id, 'LOGIN_OTP', 'USER']);
