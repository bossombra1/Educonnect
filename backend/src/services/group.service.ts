...
async function resolveGroupMembersQuery(
  ...
  switch (groupType) {
    ...
    case 'level': {
      ...
      if (level) {
        query = `SELECT DISTINCT s.user_id as id FROM students s
    ...
    case 'class': {
      if (filterData.class_id) {
        query = `SELECT s.user_id as id FROM students s
                JOIN classes c ON c.id = s.class_id
...
export async function resolveGroupMembers(groupId: number): Promise<number[]> {
  ...
  try {
    ...
    const memberIds = await resolveGroupMembersQuery(
      conn, g.group_type, g.filters, g.establishment_id
    );
...
export async function updateGroup(
  ...
  try {
    ...
    if (data.filters !== undefined) {
      fields.push('filters = ?');
      params.push(data.filters ? JSON.stringify(data.filters) : null);
    }
