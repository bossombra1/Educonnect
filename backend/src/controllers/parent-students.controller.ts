...
export async function getStudentsByParent(req: Request, res: Response): Promise<void> {
  try {
    ...
    const establishmentId = currentEstablishmentId(req);
    ...
    const [parentRows] = await pool.query<RowDataPacket[]>(
      `SELECT p.id, p.user_id, p.establishment_id
       FROM parents p JOIN users u ON u.id = p.user_id AND u.establishment_id = p.establishment_id
       WHERE p.user_id = ? AND p.establishment_id = ? LIMIT 1`,
      [parentUserId, establishmentId]
    );
    ...
    const [students] = await pool.query<RowDataPacket[]>(
      `SELECT u.id, u.first_name, u.last_name, u.matricule, u.email, u.phone, u.is_active,
       ...
       WHERE ps.parent_id = ? ORDER BY u.last_name, u.first_name`,
      [establishmentId, parentRows[0].id]
    );
  ...
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT u.id, u.first_name, u.last_name, u.matricule, u.email, u.phone,
              ...
              ps.priority, ps.is_emergency_contact
       FROM students s
       JOIN users su ON su.id = s.user_id AND su.establishment_id = s.establishment_id
       JOIN parent_student ps ON ps.student_id = s.id
       JOIN parents p ON p.id = ps.parent_id AND p.establishment_id = s.establishment_id
       JOIN users u ON u.id = p.user_id AND u.establishment_id = s.establishment_id
       WHERE s.user_id = ? AND s.establishment_id = ?
       ORDER BY u.last_name, u.first_name`,
      [studentUserId, establishmentId]
    );
...
export async function linkParentStudent(req: Request, res: Response): Promise<void> {
  try {
    const establishmentId = currentEstablishmentId(req);
    if (!establishmentId) { res.status(403).json({ success: false, error: 'Établissement non identifié.' }); return; }
    const parentUserId = Number(req.body?.parentId ?? req.body?.parent_id);
    const studentUserId = Number(req.body?.studentId ?? req.body?.student_id);
    const priority = String(req.body?.priority || 'parent1');
    const isEmergencyContact = Boolean(req.body?.isEmergencyContact ?? req.body?.is_emergency_contact ?? false);
    ...
    if (existing.length) {
      await pool.query('UPDATE parent_student SET priority = ?, is_emergency_contact = ? WHERE id = ?', [priority, isEmergencyContact ? 1 : 0, existing[0].id]);
      res.status(200).json({ success: true, message: 'Association parent-élève mise à jour.', data: { id: Number(existing[0].id), parentId: parentUserId, studentId: studentUserId, priority, isEmergencyContact } });
      return;
    }
...
export async function unlinkParentStudent(req: Request, res: Response): Promise<void> {
  try {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      ...
      [establishmentId, parentUserId, studentUserId]
    );
