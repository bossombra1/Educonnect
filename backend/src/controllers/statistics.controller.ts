...
export async function getDashboard(req: Request, res: Response): Promise<void> {
  try {
    ...
    const stats = await statisticsService.getDashboardStats(user.establishmentId);
...
export async function getMessageStats(req: Request, res: Response): Promise<void> {
  try {
    ...
    const stats = await statisticsService.getReadRateByMessage(messageId);
    res.status(200).json({ success: true, data: stats });
  } catch (err) {
    res.status(500).json({ success: false, error: (err as Error).message });
  }
...
export async function getExportStats(req: Request, res: Response): Promise<void> {
  try {
    ...
    const stats = await statisticsService.getDailyMessageStats(user.establishmentId, days);
    res.status(200).json({ success: true, data: stats });
  } catch (err) {
    res.status(500).json({ success: false, error: (err as Error).message });
  }
