...
export async function getGroupById(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (isNaN(groupId)) {
      res.status(400).json({ success: false, error: 'ID de groupe invalide.' });
      return;
    }

    const group = await groupService.getGroupById(groupId);
    if (!group) {
      res.status(404).json({ success: false, error: 'Groupe non trouvé.' });
      return;
    }
...
export async function createGroup(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!name || !group_type) {
      res.status(400).json({ success: false, error: 'Nom et type de groupe sont requis.' });
      return;
    }
    ...
    if (!VALID_GROUP_TYPES.includes(group_type)) {
      res.status(400).json({
        success: false,
        error: `Type de groupe invalide. Types acceptés: ${VALID_GROUP_TYPES.join(', ')}`
      });
      return;
    }

    const group = await groupService.createGroup(
      { name, group_type, description, filters, user_ids },
