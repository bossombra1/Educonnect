...
export async function createManagedUser(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!firstName || !lastName) {
      res.status(400).json({ success: false, error: 'Le prénom et le nom sont requis.' });
      return;
    }
    if (!['ADMIN', 'STUDENT', 'STAFF'].includes(roleName)) {
      res.status(400).json({ success: false, error: 'Rôle de création non supporté.' });
      return;
    }
    if (!password) {
      res.status(400).json({ success: false, error: 'Le mot de passe est requis.' });
      return;
    }
    ...
    if (!roles.length) {
      res.status(400).json({ success: false, error: `Le rôle ${roleName} n'existe pas dans la base.` });
      return;
    }

    let matricule = text(body.matricule);
    if (!matricule) {
      matricule = roleName === 'STUDENT'
        ? (matriculeScolaire || await generateUniqueMatricule('ELV'))
        : await generateUniqueMatricule(roleName === 'STAFF' ? 'STF' : 'ADM');
    }
    ...
    const [existingMatricule] = await pool.query<RowDataPacket[]>(
      'SELECT id FROM users WHERE matricule = ? LIMIT 1',
      [matricule]
    );
    ...
    if (roleName === 'STUDENT') {
      if (!classId) {
        res.status(400).json({ success: false, error: 'La classe est obligatoire pour créer un élève.' });
        return;
      }
      const [classes] = await pool.query<RowDataPacket[]>(
        'SELECT id FROM classes WHERE id = ? AND establishment_id = ? LIMIT 1',
        [classId, actor.establishmentId]
      );
      if (!classes.length) {
        res.status(400).json({ success: false, error: 'La classe sélectionnée n’appartient pas à votre établissement.' });
        return;
      }
    }

    const roleTitle = text(body.role_title ?? body.roleTitle);
    if (roleName === 'STAFF' && !roleTitle) {
      res.status(400).json({ success: false, error: 'La fonction du personnel est obligatoire.' });
      return;
    }
    ...
    try {
      ...
      const [userResult] = await conn.query<ResultSetHeader>(
        `INSERT INTO users (establishment_id, role_id, matricule, first_name, last_name, email, phone, password_hash, is_active)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
        [actor.establishmentId, Number(roles[0].id), matricule, firstName, lastName, email, phone, passwordHash]
      );
      ...
      if (roleName === 'STUDENT') {
        await conn.query<ResultSetHeader>(
          `INSERT INTO students (user_id, class_id, establishment_id, matricule_scolaire, status)
           VALUES (?, ?, ?, ?, 'active')`,
          [userId, classId, actor.establishmentId, matriculeScolaire || matricule]
        );
      } else if (roleName === 'STAFF') {
        await conn.query<ResultSetHeader>(
          `INSERT INTO staff (user_id, establishment_id, role_title, department)
           VALUES (?, ?, ?, ?)`,
          [userId, actor.establishmentId, roleTitle, text(body.department) || null]
        );
      } else {
        await conn.query<ResultSetHeader>(
          `INSERT INTO administrators
           (user_id, role_type, can_manage_users, can_send_broadcast, can_view_audit)
           VALUES (?, 'ADMIN', ?, ?, ?)`,
          [userId, body.can_manage_users === false ? 0 : 1, body.can_send_broadcast === false ? 0 : 1, body.can_view_audit === false ? 0 : 1]
        );
      }
      ...
      const [created] = await pool.query<RowDataPacket[]>(
        `SELECT u.id, u.first_name, u.last_name, u.matricule, u.email, u.phone, u.is_active,
                ...
                s.matricule_scolaire, s.class_id, c.name AS class_name,
                st.role_title, st.department,
                a.role_type, a.can_manage_users, a.can_send_broadcast, a.can_view_audit
         FROM users u
