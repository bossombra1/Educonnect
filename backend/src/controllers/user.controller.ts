...
export async function getAllUsers(req: Request, res: Response): Promise<void> {
  try {
    ...
    const [countRows] = await pool.query<RowDataPacket[]>(
      `SELECT COUNT(DISTINCT u.id) as total FROM users u JOIN roles r ON r.id = u.role_id ${whereClause}`,
      params
    );
    const total = countRows[0].total as number;
    const [users] = await pool.query<RowDataPacket[]>(
      `SELECT u.id, u.first_name, u.last_name, u.matricule, u.email, u.phone,
              u.is_active, u.avatar_url, u.created_at, r.name as role_name, r.label as role_label
       FROM users u JOIN roles r ON r.id = u.role_id ${whereClause}
...
export async function getUserById(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (users.length === 0) { res.status(404).json({ success: false, error: 'Utilisateur non trouvé.' }); return; }
...
export async function createUser(req: Request, res: Response): Promise<void> {
  try {
    ...
    const user = req.user as any;
    if (!first_name || !last_name) { res.status(400).json({ success: false, error: 'Les champs first_name et last_name sont requis.' }); return; }
    if (!role_id) { res.status(400).json({ success: false, error: 'Le rôle (role_id) est requis.' }); return; }
    if (!matricule) { res.status(400).json({ success: false, error: 'Le matricule est requis.' }); return; }

    const pool = getPool();
    const [roles] = await pool.query<RowDataPacket[]>('SELECT id, name FROM roles WHERE id = ?', [role_id]);
    if (roles.length === 0) { res.status(400).json({ success: false, error: 'Rôle non trouvé.' }); return; }
    ...
    if (email) {
      ...
      if (existingEmail.length > 0) { res.status(409).json({ success: false, error: 'Un utilisateur avec cet email existe déjà.' }); return; }
    }
    const [existingMat] = await pool.query<RowDataPacket[]>('SELECT id FROM users WHERE matricule = ?', [matricule]);
    if (existingMat.length > 0) { res.status(409).json({ success: false, error: 'Un utilisateur avec ce matricule existe déjà.' }); return; }
    ...
    try {
      ...
      if (roleName === 'STUDENT') {
        ...
        if (!class_id) throw new Error('Le champ class_id est requis pour un élève.');
        const [classes] = await conn.query<RowDataPacket[]>('SELECT id FROM classes WHERE id = ? AND establishment_id = ?', [class_id, user.establishmentId]);
        if (classes.length === 0) throw new Error('La classe sélectionnée n’appartient pas à votre établissement.');
      }
      ...
      if (roleName === 'STUDENT') {
        ...
        await conn.query<ResultSetHeader>(
          `INSERT INTO students (user_id, class_id, establishment_id, matricule_scolaire, status) VALUES (?, ?, ?, ?, 'active')`,
      ...
      if (roleName === 'STAFF') {
        ...
        if (!role_title) throw new Error('Le champ role_title est requis pour le personnel.');
        await conn.query<ResultSetHeader>(
          `INSERT INTO staff (user_id, establishment_id, role_title, department) VALUES (?, ?, ?, ?)`,
...
export async function deleteUser(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (result.affectedRows === 0) { res.status(404).json({ success: false, error: 'Utilisateur non trouvé.' }); return; }
...
export async function searchUsers(req: Request, res: Response): Promise<void> {
  try {
    const user = req.user as any; const pool = getPool(); const query = (req.query.q as string || '').trim(); const limit = Math.min(50, parseInt(req.query.limit as string) || 20);
